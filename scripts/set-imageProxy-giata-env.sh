#!/usr/bin/env bash
# Set GIATA_USERNAME / GIATA_PASSWORD on imageProxy from SSM (no full serverless deploy).
set -euo pipefail

REGION="${AWS_REGION:-eu-west-1}"
FUNCTION_NAME="${1:-hotel-search-dev-imageProxy}"

echo "Fetching GIATA credentials from SSM..."
export GIATA_USERNAME="$(aws ssm get-parameter \
  --region "$REGION" \
  --name /al-rais/dev/giata/username \
  --with-decryption \
  --query Parameter.Value \
  --output text)"
export GIATA_PASSWORD="$(aws ssm get-parameter \
  --region "$REGION" \
  --name /al-rais/dev/giata/password \
  --with-decryption \
  --query Parameter.Value \
  --output text)"
