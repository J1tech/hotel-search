...
// English address formatting for Place Details (does not change ranking).
const ENGLISH_REGION_CODE = "US";
...
export function getPlaceDetailsCacheKey(placeId, languageCode) {
    ...
    return `geo:places:details:v2:${id}:${lang}`;
}
...
export async function autocompletePlaces({ input, sessionToken, countryBias, languageCode }) {
    ...
    const lang = resolveLanguageCode(languageCode);
    const cacheKey = getAutocompleteCacheKey({ input: query, countryBias, languageCode: lang });
    try {
    ...
    const body = { input: query, languageCode: lang };
...
export async function getPlaceDetails({ placeId, sessionToken, languageCode }) {
    ...
    const params = { languageCode: lang };
    if (lang === DEFAULT_LANGUAGE_CODE) params.regionCode = ENGLISH_REGION_CODE;
