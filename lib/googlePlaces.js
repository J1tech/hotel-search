...
export { distanceMeters } from "./distance.js";
...
const OVERVIEW_FIELD_MASK = "id,displayName,location,rating,userRatingCount";

function overviewCacheKey(lookupKey) {
    return `googleReviewsOverview:${lookupKey}`;
}

export function overviewPayload(details) {
    const count = Number(details?.userRatingCount);
    return {
        rating: details?.rating ?? null,
        userRatingCount: Number.isFinite(count) ? count : null,
    };
}
...
export async function getGooglePlaceRating(placeId) {
    const { data } = await axios.get(`${PLACES_BASE}/places/${encodeURIComponent(placeId)}`, {
        timeout: 15000,
        headers: googleHeaders(OVERVIEW_FIELD_MASK),
    });
    const location = readGoogleLocation(data?.location);
    return {
        placeId: data?.id || placeId,
...
export async function getCachedOverview(lookupKey) {
    if (!lookupKey) return null;
    try {
        const raw = await redis.get(overviewCacheKey(lookupKey));
        return raw ? JSON.parse(raw) : null;
    } catch (error) {
        console.error("Redis GET overview error:", error.message);
        return null;
    }
}

export async function getCachedOverviewByKeys(pairKey, hashKey) {
    const [pairHit, hashHit] = await Promise.all([
        getCachedOverview(pairKey),
        getCachedOverview(hashKey),
    ]);
    return pairHit || hashHit;
}

export async function cacheOverview(lookupKey, payload) {
    if (!lookupKey) return;
    try {
        await redis.set(
            overviewCacheKey(lookupKey),
            JSON.stringify(payload),
            "EX",
            REVIEWS_CACHE_TTL_SECONDS
        );
    } catch (error) {
        console.error("Redis SET overview error:", error.message);
    }
}
