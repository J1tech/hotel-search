...
function extractPromoForAlert(snapshot, bookingRecord) {
  const candidates = [
    snapshot?.promo,
    snapshot?.preBookData?.promo,
    bookingRecord?.promo,
  ];
  for (const candidate of candidates) {
    ...
    try {
      ...
      if (parsed?.code || parsed?.promoCode) return parsed;
    } catch {
      /* skip invalid promo JSON */
    }
  }
  return null;
}
...
export async function sendHotelOpsAlert({
  ...
  const snapshot = safeJson(record?.snapshot || bookingRecord?.checkoutSnapshot || null);
  const promo = extractPromoForAlert(snapshot, safeJson(bookingRecord));
