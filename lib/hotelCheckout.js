...
export async function refreshPaymentStatus(record) {
  if (!record?.orderReference) return record;
  if (record.paymentStatus === "captured" || record.paymentStatus === "expired") return record;
  ...
  if (mapped === record.paymentStatus) return record;
  return patchCheckoutSnapshot(record.bookingKey, { paymentStatus: mapped });
}
...
export async function stampConfirmInProgress(checkoutId) {
  const current = await loadCheckoutById(checkoutId);
  ...
  await patchCheckoutSnapshot(current.bookingKey, { confirmStatus: "in_progress" });
  return current;
}
