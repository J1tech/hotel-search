...
function isNonProdStage() {
  const stage = String(process.env.STAGE || process.env.SERVERLESS_STAGE || "").toLowerCase();
  return stage !== "prod" && stage !== "production";
}

function isDevHttpRedirectHost(hostname) {
  ...
  if (isNonProdStage() && host.includes(".s3-website.") && host.endsWith(".amazonaws.com")) {
    return true;
  }
  return false;
}

/** Reject API Gateway / execute-api origins (Vite proxy changeOrigin can send these). */
function isTrustedCheckoutOrigin(origin) {
  try {
    const url = new URL(origin);
    const host = url.hostname.toLowerCase();
    if (host.includes("execute-api") && host.includes("amazonaws.com")) return false;
    if (isDevHttpRedirectHost(host)) return true;
    if (url.protocol === "https:") return true;
    return isNonProdStage() && url.protocol === "http:" && isDevHttpRedirectHost(host);
  } catch {
    return false;
  }
}
...
/**
 * Network redirect after hosted pay.
 * Prod: HTTPS only. Dev/QA: also allow http localhost and S3 static website hosts.
 */
export function normalizeRedirectUrl(value) {
  const url = String(value || "").trim();
  if (!url) return null;
  if (/^https:\/\//i.test(url)) return url;
  if (isNonProdStage() && /^http:\/\//i.test(url)) {
    try {
      if (isDevHttpRedirectHost(new URL(url).hostname)) return url;
    } catch {
      /* ignore */
    }
  }
...
const CHECKOUT_PATH = "/hotels/checkout";

export function headerValue(event, name) {
  const headers = event?.headers || {};
  const key = Object.keys(headers).find((k) => k.toLowerCase() === String(name).toLowerCase());
  return key ? String(headers[key] || "").trim() : "";
}

/** Mint on Pay click — header or ?mint=1 (query kept for backward compatibility). */
export function parseCheckoutMintRequest(event) {
  const qs = event?.queryStringParameters || {};
  const mintHeader = headerValue(event, "x-checkout-mint");
  const shouldMint =
    mintHeader === "1" ||
    mintHeader.toLowerCase() === "true" ||
    qs.mint === "1" ||
    qs.mint === "true";
  return { shouldMint };
}
...
/**
 * Return URL for Network redirect — derived from browser headers (no FE param required).
 * 1. Referer (full checkout page URL)
 * 2. Origin + canonical checkout path
 * 3. Optional override: X-Checkout-Redirect-Url or ?redirectUrl=
 */
export function resolveCheckoutRedirectUrl(event, checkoutId) {
  const qs = event?.queryStringParameters || {};
  const explicit =
    normalizeRedirectUrl(headerValue(event, "x-checkout-redirect-url")) ||
    normalizeRedirectUrl(qs.redirectUrl);
  if (explicit) return explicit;

  const referer = headerValue(event, "referer");
  if (referer) {
    try {
      const url = new URL(referer.split("#")[0]);
      if (url.pathname === CHECKOUT_PATH && url.searchParams.get("checkoutId") === checkoutId) {
        const normalized = normalizeRedirectUrl(url.toString());
  ...
  if (origin && checkoutId && isTrustedCheckoutOrigin(origin)) {
    const built = `${origin.replace(/\/$/, "")}${CHECKOUT_PATH}?checkoutId=${encodeURIComponent(checkoutId)}`;
    return normalizeRedirectUrl(built);
  }
...
async function createNgeniusInvoice({
  ...
  const invoiceRedirect = normalizeRedirectUrl(redirectUrl);
  if (invoiceRedirect) body.redirectUrl = invoiceRedirect;
...
async function createNgeniusHostedOrder({
  ...
  const hostedRedirect = normalizeRedirectUrl(redirectUrl);
  if (hostedRedirect) {
    body.merchantAttributes = {
      redirectUrl: hostedRedirect,
      skipConfirmationPage: true,
      cancelUrl: hostedRedirect,
    };
  }
...
export async function reconcileStaleHotelConfirm(record) {
  ...
  const patched = await patchCheckoutSnapshot(record.bookingKey, {
    confirmStatus: "confirm_failed",
    confirmStartedAt: "",
  });
  try {
    const { sendHotelOpsAlert } = await import("./opsBookingAlert.js");
    await sendHotelOpsAlert({
      scenario: "booking_failed",
      record: patched || record,
      reason: "stale_confirm_in_progress",
    });
  } catch (err) {
    console.warn("hotel ops alert (stale confirm) failed:", err?.message || err);
  }
  return patched;
}
